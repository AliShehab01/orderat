...
Object.assign(OA, (()=>{
 ...
 function parse(text,s,reference=OA.localDate()){
  ...
  const tm=str.match(/(?:\bat\s*|الساعة\s*|الساعه\s*|@\s*)(\d{1,2})(?::(\d{2}))?\s*(am|pm|a\.m\.|p\.m\.|صباحا|صباح|مساء|مسا|العصر|ص|م)?/i)||str.match(/\b(\d{1,2})(?::(\d{2}))?\s*(am|pm)\b/i);
  if(tm){let h=Number(tm[1]),m=Number(tm[2]||0),amp=tm[3]||'';if((amp&&h>=1&&h<=12)||(!amp&&h>=13&&h<=23)){if(/pm|p\.m|مساء|مسا|العصر|^م$/.test(amp)&&h<12)h+=12;if(/am|a\.m|صباح|^ص$/.test(amp)&&h===12)h=0;if(m<60)out.time=String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')}}
