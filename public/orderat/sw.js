const CACHE='orderat-prototype-v2';
const FILES=['./','./index.html','./style.css','./plans.css','./core.js','./app.js','./ui.js','./flows.js','./plans.js','./webmcp.js','./favicon.svg'];
self.addEventListener('install',e=>{e.waitUntil(caches.open(CACHE).then(c=>c.addAll(FILES)).then(()=>self.skipWaiting()))});
self.addEventListener('activate',e=>{e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k.startsWith('order-agent-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim()))});
